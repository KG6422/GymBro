...
public class MainPanel extends JPanel {
	...
	private ArrayList<MuscleWork> mWork = new ArrayList<>();
	...
	public void Change(GymDay inDay) {
		...
		//DefaultTableModel model = new DefaultTableModel(day.toObjectArray(), headers);
		...
		changeDayTypeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int response = JOptionPane.showOptionDialog(new JFrame(),
						"Changing the Type of Day will delete all activities for the day and reset it.\nGym Bro will also save all unsaved data.", "Gym Bro Override", JOptionPane.YES_NO_OPTION,
						JOptionPane.PLAIN_MESSAGE, null, responses, responses[1]);
				if (response == 0) {
	...
	/**
	 * Index 0 : String name of exercise
	 * Index 1 : Sets in String form 
	 * Index 2 : Reps in String form 
	 * Index 3 : Weight in String form 
	 * Index 4 : null - KRP calculation 
	 * Index 5 : muscles in muscle[] form 
	 * Index 6 : null - Eventual Delete Button
	 * 
	...
	private JPanel dealWithMusclePanel() {
		...
		for (muscle m : list) {
			...
			jTitle.setFont(new Font("LABEL", Font.BOLD, 12));
